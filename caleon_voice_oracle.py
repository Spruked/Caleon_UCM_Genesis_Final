"""
Caleon Voice Oracle - Decision Engine for Voice Selection
Caleon's brain for choosing her optimal voice signature.
"""
...
class VoiceSignature:
    ...
    def calculate_fitness(self, content_vector: np.ndarray, context: Dict) -> float:
        ...
        # 1. Semantic similarity - use oracle's vectorization
        oracle = CaleonVoiceOracle.__new__(CaleonVoiceOracle)  # Create instance without __init__
        tag_vector = oracle._vectorize_tags(self.semantic_tags)
        semantic_score = np.dot(content_vector, tag_vector) / (np.linalg.norm(content_vector) * np.linalg.norm(tag_vector)) if np.linalg.norm(tag_vector) > 0 else 0
    ...
    def _score_context_match(self, context: Dict) -> float:
        ...
        # STRONG PENALTY: Authoritative voice when intimacy is high
        if context.get("audience_intimacy", 0.0) > 0.7 and "authoritative" in self.semantic_tags:
            score *= 0.4  # 60% penalty for mismatch

        # STRONG PENALTY: Intimate voice when technical density is high
        if context.get("technical_density", 0.0) > 0.7 and "intimate" in self.semantic_tags:
            score *= 0.3  # 70% penalty for mismatch

        # EMOTIONAL TONE MATCHING: Strong boost for emotional context matching
        emotional_tone = context.get("emotional_tone", "neutral")

        # For sad/emotional content, strongly prefer intimate/soft voices
        if emotional_tone in ["sad", "emotional", "personal"] and "intimate" in self.semantic_tags:
            score *= 1.5  # 50% bonus for emotional match
        elif emotional_tone in ["sad", "emotional", "personal"] and "excited" in self.semantic_tags:
            score *= 0.6  # 40% penalty for mismatch

        # For hopeful/inspiring content, strongly prefer enthusiastic/upbeat voices
        if emotional_tone in ["hopeful", "inspiring", "motivational"] and "excited" in self.semantic_tags:
            score *= 1.4  # 40% bonus for motivational match
        elif emotional_tone in ["hopeful", "inspiring", "motivational"] and "intimate" in self.semantic_tags:
            score *= 0.7  # 30% penalty for mismatch
...
class CaleonVoiceOracle:
    ...
    def _load_voice_registry(self) -> List[VoiceSignature]:
        ...
        try:
            ...
            # Create default SKG if it doesn't exist
            self._create_default_skg()
            with open(self.skg_path, 'r') as f:
                skg = json.load(f)
    ...
    def _create_default_skg(self):
        """Create default SKG structure"""
        default_skg = {
            "caleon_identity": {
                "name": "Caleon",
                "instance_id": "caleon_primary",
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        # Simple keyword extraction (in production, use spaCy or similar)
    ...
    def generate_new_voice_signature(self, from_content: str, performance_hint: float) -> str:
        ...
        new_voice = VoiceSignature(
            signature_id=voice_id,
            base_persona="caleon_base",
            pitch_shift=new_voice_config.get("pitch_shift", 1.0),
            speaking_rate=new_voice_config.get("speaking_rate", 1.0),
            formant_shifts=new_voice_config.get("formant_shifts", {}),
            breathiness=new_voice_config.get("breathiness", 0.3),
            vocal_fry=new_voice_config.get("vocal_fry", 0.1),
            nasality=new_voice_config.get("nasality", 0.0),
            reverb=new_voice_config.get("reverb"),
            semantic_tags=new_voice_config.get("semantic_tags", []),
            success_score=new_voice_config.get("success_score", 0.7)
        )
    ...
    def _derive_voice_from_content(self, analysis: Dict, hint: float) -> Dict:
        ...
        # Clean semantic tags from content analysis
        semantic_tags = []
        if analysis["avg_word_length"] > 6:
            semantic_tags.append("long_words")
        else:
            semantic_tags.append("short_words")

        if analysis["sentiment"] > 2:
            semantic_tags.append("positive")
        elif analysis["sentiment"] < -2:
            semantic_tags.append("negative")
        else:
            semantic_tags.append("neutral")

        if analysis["complexity"] > 0.7:
            semantic_tags.append("high_complexity")
        else:
            semantic_tags.append("low_complexity")

        if analysis["length"] > 200:
            semantic_tags.append("long_content")
        else:
            semantic_tags.append("short_content")

        config["semantic_tags"] = semantic_tags
