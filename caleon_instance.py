"""
Caleon Personal POM Instance
Caleon gets her own isolated POM with her learned voices, separate from the main system.
"""
...
from self_modifying_pom import SelfModifyingPOM
...
class CaleonPOMInstance:
    ...
    def _setup_workspace(self):
        ...
        if not os.path.exists(self.workspace_dir):
            ...
            # Copy base POM components (if they exist)
            base_pom_dir = "cerebral_cortex"
            if os.path.exists(base_pom_dir):
                shutil.copytree(
                    base_pom_dir,
                    f"{self.workspace_dir}/cerebral_cortex",
                    ignore=shutil.ignore_patterns('*.pyc', '__pycache__'),
                    dirs_exist_ok=True
                )

            # Copy base SKG but make it personal
            base_skg = "skg_caleon.json"
            if os.path.exists(base_skg):
                shutil.copy(base_skg, f"{self.workspace_dir}/skg_caleon_personal.json")
            else:
    ...
    def export_voice_dna(self) -> Dict:
        ...
        return {
            ...
            "voice_registry": [v.__dict__ for v in self.oracle.voice_registry],
            ...
            "pom_modification_log": self.pom.get_modification_history(),
            "export_timestamp": time.time()
        }

    def import_voice_dna(self, dna: Dict):
        """Import Caleon's learned voice preferences"""

        print(f"🧬 Importing voice DNA for instance: {dna.get('instance_id', 'unknown')}")

        # Update evolution log
        if 'evolution_log' in dna:
            self.evolution_log.extend(dna['evolution_log'])

        # Update oracle parameters
        if 'learning_params' in dna:
            self.oracle.learning_rate = dna['learning_params'].get('learning_rate', 0.1)
            self.oracle.exploration_rate = dna['learning_params'].get('exploration_rate', 0.15)

        # Update performance log
        if 'performance_log' in dna:
            self.oracle.performance_log.extend(dna['performance_log'])

        print("   → Voice DNA imported successfully")

    def get_instance_status(self) -> Dict:
        """Get current status of Caleon's instance"""

        return {
            "instance_id": self.instance_id,
            "workspace_dir": self.workspace_dir,
            "voice_count": len(self.oracle.voice_registry),
            "evolution_events": len(self.evolution_log),
            "performance_records": len(self.oracle.performance_log),
            "oracle_exploration_rate": self.oracle.exploration_rate,
            "total_modifications": len(self.pom.get_modification_history())
        }

    def reset_instance(self):
        """Reset Caleon's instance to default state"""

        print(f"🔄 Resetting Caleon instance: {self.instance_id}")
